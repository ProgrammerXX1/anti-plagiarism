...
bool SearchEngine::load(const std::string& index_dir) {
    ...
    in.read(magic, 4);
    if (!in || magic[0] != 'P' || magic[1] != 'L' || magic[2] != 'A' || magic[3] != 'G') return false;
    ...
    if (!read_u32(in, N_docs) || !read_u64(in, N_post9) || !read_u64(in, N_post13)) return false;
    ...
    for (std::size_t i = 0; i < (std::size_t)N_docs; ++i) {
        ...
        if (!read_u32(in, tok_len) || !read_u64(in, hi) || !read_u64(in, lo)) return false;
        DocMeta dm{};
        dm.tok_len = tok_len;
        dm.bm25_len = tok_len;
        dm.simhash_hi = hi;
    ...
    if (version == 1) {
        ...
        for (std::size_t i = 0; i < (std::size_t)N_post9; ++i) {
            ...
            std::uint32_t did = 0;
            std::uint32_t pos = 0;
            if (!read_u64(in, h) || !read_u32(in, did) || !read_u32(in, pos)) return false;
...
int SearchEngine::search_text(
    ...
    if (!build_query_shingles(text_utf8, norm, spans, qsh, normalize_input)) return 0;

    // scoring uses unique hashes
